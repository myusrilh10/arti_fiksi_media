...
export default function HeroCarousel({ articles }: HeroCarouselProps) {
    ...
    return (
        <div className="relative w-full overflow-hidden group">
            ...
            <div className="absolute inset-y-0 left-0 flex items-center justify-start z-30 px-2 md:px-6 pointer-events-none">
                <button
                    onClick={prevSlide}
                    className="p-3 md:p-4 rounded-full bg-black/40 backdrop-blur-md text-white border border-white/20 hover:bg-lemon-lime hover:text-black hover:border-lemon-lime transition-all duration-300 pointer-events-auto opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0"
                    aria-label="Previous slide"
                >
                    <ChevronLeft className="w-6 h-6" />
                </button>
            </div>
            <div className="absolute inset-y-0 right-0 flex items-center justify-end z-30 px-2 md:px-6 pointer-events-none">
                <button
                    onClick={nextSlide}
                    className="p-3 md:p-4 rounded-full bg-black/40 backdrop-blur-md text-white border border-white/20 hover:bg-lemon-lime hover:text-black hover:border-lemon-lime transition-all duration-300 pointer-events-auto opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0"
